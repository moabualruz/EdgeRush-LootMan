...
class ApplicationQuestionFileControllerTest : UnitTest() {
    private lateinit var service: ApplicationQuestionFileCrudService
    private lateinit var paginationProperties: PaginationProperties
    private lateinit var controller: ApplicationQuestionFileController

    @BeforeEach
    fun setup() {
        service = mockk()
        paginationProperties = PaginationProperties(defaultPageSize = 20, maxPageSize = 100)
        controller = ApplicationQuestionFileController(service, paginationProperties)
    }
    ...
    inner class CrudTests {
        ...
        fun `should create with 201`() {
            ...
            controller.create(CreateApplicationQuestionFileRequest(1L)).statusCode shouldBe HttpStatus.CREATED
        }

        @Test
        fun `should update`() {
            every { service.update(1L, any()) } returns createResponse()
            controller.update(1L, UpdateApplicationQuestionFileRequest()).id shouldBe 1L
        }
