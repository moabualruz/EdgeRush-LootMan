...
class RaiderRaidProgressControllerTest : UnitTest() {
    private lateinit var service: RaiderRaidProgressCrudService
    private lateinit var paginationProperties: PaginationProperties
    private lateinit var controller: RaiderRaidProgressController

    @BeforeEach
    fun setup() {
        service = mockk()
        paginationProperties = PaginationProperties(defaultPageSize = 20, maxPageSize = 100)
        controller = RaiderRaidProgressController(service, paginationProperties)
    }
    ...
    inner class CrudTests {
        ...
        fun `should create with 201`() {
            ...
            controller.create(CreateRaiderRaidProgressRequest(1L, "Nerub-ar Palace", "Mythic")).statusCode shouldBe HttpStatus.CREATED
        }

        @Test
        fun `should update`() {
            every { service.update(1L, any()) } returns createResponse()
            controller.update(1L, UpdateRaiderRaidProgressRequest()).id shouldBe 1L
        }
