...
class RaiderCrestCountControllerTest : UnitTest() {
    private lateinit var service: RaiderCrestCountCrudService
    private lateinit var paginationProperties: PaginationProperties
    private lateinit var controller: RaiderCrestCountController

    @BeforeEach
    fun setup() {
        service = mockk()
        paginationProperties = PaginationProperties(defaultPageSize = 20, maxPageSize = 100)
        controller = RaiderCrestCountController(service, paginationProperties)
    }
    ...
    inner class CrudTests {
        ...
        fun `should create with 201`() {
            ...
            controller.create(CreateRaiderCrestCountRequest(1L, "Weathered")).statusCode shouldBe HttpStatus.CREATED
        }

        @Test
        fun `should update`() {
            every { service.update(1L, any()) } returns createResponse()
            controller.update(1L, UpdateRaiderCrestCountRequest()).id shouldBe 1L
        }
