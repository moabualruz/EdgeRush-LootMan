...
class RaiderTrackItemControllerTest : UnitTest() {
    private lateinit var service: RaiderTrackItemCrudService
    private lateinit var paginationProperties: PaginationProperties
    private lateinit var controller: RaiderTrackItemController

    @BeforeEach
    fun setup() {
        service = mockk()
        paginationProperties = PaginationProperties(defaultPageSize = 20, maxPageSize = 100)
        controller = RaiderTrackItemController(service, paginationProperties)
    }
    ...
    inner class CrudTests {
        ...
        fun `should create with 201`() {
            ...
            controller.create(CreateRaiderTrackItemRequest(1L, "hero")).statusCode shouldBe HttpStatus.CREATED
        }

        @Test
        fun `should update`() {
            every { service.update(1L, any()) } returns createResponse()
            controller.update(1L, UpdateRaiderTrackItemRequest()).id shouldBe 1L
        }
