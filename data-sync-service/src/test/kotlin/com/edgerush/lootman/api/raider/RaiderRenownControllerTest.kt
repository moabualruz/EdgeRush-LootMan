...
class RaiderRenownControllerTest : UnitTest() {
    private lateinit var service: RaiderRenownCrudService
    private lateinit var paginationProperties: PaginationProperties
    private lateinit var controller: RaiderRenownController

    @BeforeEach
    fun setup() {
        service = mockk()
        paginationProperties = PaginationProperties(defaultPageSize = 20, maxPageSize = 100)
        controller = RaiderRenownController(service, paginationProperties)
    }
    ...
    inner class CrudTests {
        ...
        fun `should create with 201`() {
            ...
            controller.create(CreateRaiderRenownRequest(1L, "The Weaver")).statusCode shouldBe HttpStatus.CREATED
        }

        @Test
        fun `should update`() {
            every { service.update(1L, any()) } returns createResponse()
            controller.update(1L, UpdateRaiderRenownRequest()).id shouldBe 1L
        }
