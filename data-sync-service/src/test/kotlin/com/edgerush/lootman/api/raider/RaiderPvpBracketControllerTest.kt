...
class RaiderPvpBracketControllerTest : UnitTest() {
    private lateinit var service: RaiderPvpBracketCrudService
    private lateinit var paginationProperties: PaginationProperties
    private lateinit var controller: RaiderPvpBracketController

    @BeforeEach
    fun setup() {
        service = mockk()
        paginationProperties = PaginationProperties(defaultPageSize = 20, maxPageSize = 100)
        controller = RaiderPvpBracketController(service, paginationProperties)
    }
    ...
    inner class CrudTests {
        ...
        fun `should create with 201`() {
            ...
            controller.create(CreateRaiderPvpBracketRequest(1L, "2v2")).statusCode shouldBe HttpStatus.CREATED
        }

        @Test
        fun `should update`() {
            every { service.update(1L, any()) } returns createResponse()
            controller.update(1L, UpdateRaiderPvpBracketRequest()).id shouldBe 1L
        }
