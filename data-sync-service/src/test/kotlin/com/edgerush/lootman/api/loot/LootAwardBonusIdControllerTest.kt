...
class LootAwardBonusIdControllerTest : UnitTest() {
    private lateinit var service: LootAwardBonusIdCrudService
    private lateinit var paginationProperties: PaginationProperties
    private lateinit var controller: LootAwardBonusIdController

    @BeforeEach
    fun setup() {
        service = mockk()
        paginationProperties = PaginationProperties(defaultPageSize = 20, maxPageSize = 100)
        controller = LootAwardBonusIdController(service, paginationProperties)
    }
    ...
    inner class CrudTests {
        ...
        fun `should create with 201`() {
            ...
            controller.create(CreateLootAwardBonusIdRequest(1L)).statusCode shouldBe HttpStatus.CREATED
        }

        @Test
        fun `should update`() {
            every { service.update(1L, any()) } returns createResponse()
            controller.update(1L, UpdateLootAwardBonusIdRequest()).id shouldBe 1L
        }
