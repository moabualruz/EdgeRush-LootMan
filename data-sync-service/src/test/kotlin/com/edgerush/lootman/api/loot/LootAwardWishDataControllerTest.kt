...
class LootAwardWishDataControllerTest : UnitTest() {
    private lateinit var service: LootAwardWishDataCrudService
    private lateinit var paginationProperties: PaginationProperties
    private lateinit var controller: LootAwardWishDataController

    @BeforeEach
    fun setup() {
        service = mockk()
        paginationProperties = PaginationProperties(defaultPageSize = 20, maxPageSize = 100)
        controller = LootAwardWishDataController(service, paginationProperties)
    }
    ...
    inner class CrudTests {
        ...
        fun `should create with 201`() {
            ...
            controller.create(CreateLootAwardWishDataRequest(1L)).statusCode shouldBe HttpStatus.CREATED
        }

        @Test
        fun `should update`() {
            every { service.update(1L, any()) } returns createResponse()
            controller.update(1L, UpdateLootAwardWishDataRequest()).id shouldBe 1L
        }
