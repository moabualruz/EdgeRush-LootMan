...
class SimulationServiceTest : UnitTest() {
    ...
    inner class SubmitSimulation {
        ...
        @Test
        fun `should use default parameters when not specified`() = runBlocking {
            // Arrange
            val guildId = "guild-123"
        ...
        @Test
        fun `should accept custom iterations and fight length`() = runBlocking {
            // Arrange
            val guildId = "guild-123"
    ...
    inner class ExecutePendingSimulations {
        ...
        @Test
        fun `should continue executing remaining simulations when one throws exception`() = runBlocking {
            // Arrange
        ...
        @Test
        fun `should save results when profileId exists after successful execution`() = runBlocking {
            // Arrange
            val profile = createProfile()
            val pendingRequest = SimulationRequest.create(profile = profile).withId(1L)
            val results = listOf(
        ...
        @Test
        fun `should not save results when profileId is null after successful execution`() = runBlocking {
            // Arrange
            val profile = createProfile()
            val pendingRequest = SimulationRequest.create(profile = profile).withId(1L)
            val results = listOf(
        ...
        @Test
        fun `should handle failure with null error message`() = runBlocking {
            // Arrange
            val profile = createProfile()
            val pendingRequest = SimulationRequest.create(profile = profile).withId(1L)
