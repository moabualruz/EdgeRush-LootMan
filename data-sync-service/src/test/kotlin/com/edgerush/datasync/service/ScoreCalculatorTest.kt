...
class ScoreCalculatorTest {
    ...
    private val calculator =
        ScoreCalculator(mockDataTransformer, mockModifierService, mockPerformanceService, mockRaidbotsService, fixedClock)
    ...
    fun `calculates FLPS breakdowns matching walkthrough`() {
        ...
        val specAverages =
            (logsData["spec_averages"] as Map<*, *>).mapValues { (_, value) ->
        ...
        val baseline =
            (droptimizerData["baseline_dps"] as Map<*, *>).mapValues { (_, value) ->
        ...
        val inputs =
            rosterEntries.map { entry ->
                ...
                val lastAwards =
                    (e["last_awards"] as? List<*>)?.mapNotNull { award ->
                        val aw = award as Map<*, *>
                        val tier =
                            when (aw["tier"] as String) {
                                "A" -> LootTier.A
                                "B" -> LootTier.B
                                else -> LootTier.C
                            }
