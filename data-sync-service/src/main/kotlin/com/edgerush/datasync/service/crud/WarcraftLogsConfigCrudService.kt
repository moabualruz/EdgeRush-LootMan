package com.edgerush.datasync.service.crud

import com.edgerush.datasync.api.dto.request.CreateWarcraftLogsConfigRequest
import com.edgerush.datasync.api.dto.request.UpdateWarcraftLogsConfigRequest
import com.edgerush.datasync.api.dto.response.WarcraftLogsConfigResponse
import com.edgerush.datasync.entity.warcraftlogs.WarcraftLogsConfigEntity
import com.edgerush.datasync.repository.warcraftlogs.WarcraftLogsConfigRepository
import com.edgerush.datasync.service.mapper.WarcraftLogsConfigMapper
import com.edgerush.datasync.security.AuthenticatedUser
import com.edgerush.datasync.api.exception.ResourceNotFoundException
import com.edgerush.datasync.service.AuditLogger
import org.springframework.data.domain.Page
import org.springframework.data.domain.Pageable
import org.springframework.stereotype.Service
import org.springframework.transaction.annotation.Transactional

@Service
class WarcraftLogsConfigCrudService(
    private val repository: WarcraftLogsConfigRepository,
    private val mapper: WarcraftLogsConfigMapper
,
    private val auditLogger: AuditLogger
) : CrudService<WarcraftLogsConfigEntity, String, CreateWarcraftLogsConfigRequest, UpdateWarcraftLogsConfigRequest, WarcraftLogsConfigResponse> {

    override fun findAll(pageable: Pageable): Page<WarcraftLogsConfigResponse> {
        val allEntities = repository.findAll().toList()
        val start = (pageable.pageNumber * pageable.pageSize).coerceAtMost(allEntities.size)
        val end = (start + pageable.pageSize).coerceAtMost(allEntities.size)
        val pageContent = allEntities.subList(start, end)
        return org.springframework.data.domain.PageImpl(
            pageContent.map(mapper::toResponse),
            pageable,
            allEntities.size.toLong()
        )
    }

    override fun findById(id: String): WarcraftLogsConfigResponse {
        val entity = repository.findById(id)
            .orElseThrow { ResourceNotFoundException("WarcraftLogsConfig not found with id: $id") }
        return mapper.toResponse(entity)
    }

    @Transactional
    override fun create(request: CreateWarcraftLogsConfigRequest, user: AuthenticatedUser): WarcraftLogsConfigResponse {
        val entity = mapper.toEntity(request)
        val saved = repository.save(entity)

        auditLogger.logCreate(
            entityType = "WarcraftLogsConfig",
            entityId = saved.guildId!!,
            user = user
        )

        return mapper.toResponse(saved)
    }

    @Transactional
    override fun update(id: String, request: UpdateWarcraftLogsConfigRequest, user: AuthenticatedUser): WarcraftLogsConfigResponse {
        val existing = repository.findById(id)
            .orElseThrow { ResourceNotFoundException("WarcraftLogsConfig not found with id: $id") }
        val updated = mapper.updateEntity(existing, request)
        val saved = repository.save(updated)

        auditLogger.logUpdate(
            entityType = "WarcraftLogsConfig",
            entityId = id,
            user = user
        )

        return mapper.toResponse(saved)
    }

    @Transactional
    override fun delete(id: String, user: AuthenticatedUser) {
        if (!repository.existsById(id)) {
            throw ResourceNotFoundException("WarcraftLogsConfig not found with id: $id")
        }

        repository.deleteById(id)

        auditLogger.logDelete(
            entityType = "WarcraftLogsConfig",
            entityId = id,
            user = user
        )
    }

    override fun validateAccess(entity: WarcraftLogsConfigEntity, user: AuthenticatedUser) {
        // Add guild-based access control if needed
    }
}
