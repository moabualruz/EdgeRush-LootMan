...
class RaiderService(
    ...
    fun findRaider(
        name: String,
        realm: String,
    ): RaiderEntity? = raiderRepository.findByCharacterNameAndRealm(name, realm).orElse(null)

    fun upsertCharacter(record: RaiderRecord): RaiderEntity {
        val existing =
            record.wowauditId?.let { id ->
        ...
        if (record.gearItems.isNotEmpty()) {
            raiderGearItemRepository.saveAll(
                record.gearItems.map { gear ->
        ...
        record.statistics?.let { stats ->
            ...
            if (stats.warcraftLogs.isNotEmpty()) {
                raiderWarcraftLogRepository.saveAll(
                    stats.warcraftLogs.map { log ->
            ...
            if (stats.trackItems.isNotEmpty()) {
                raiderTrackItemRepository.saveAll(
                    stats.trackItems.map { item ->
            ...
            if (stats.crestCounts.isNotEmpty()) {
                raiderCrestCountRepository.saveAll(
                    stats.crestCounts.map { crest ->
            ...
            if (stats.vaultSlots.isNotEmpty()) {
                raiderVaultSlotRepository.saveAll(
                    stats.vaultSlots.map { slot ->
            ...
            if (stats.renownLevels.isNotEmpty()) {
                raiderRenownRepository.saveAll(
                    stats.renownLevels.map { renown ->
                        RaiderRenownEntity(
                            raiderId = raiderId,
            ...
            if (stats.raidProgress.isNotEmpty()) {
                raiderRaidProgressRepository.saveAll(
                    stats.raidProgress.map { progress ->
        ...
        if (record.pvpBrackets.isNotEmpty()) {
            raiderPvpBracketRepository.saveAll(
                record.pvpBrackets.map { bracket ->
    ...
    fun findAllCharacters(): List<RaiderEntity> = raiderRepository.findAll().toList()

    fun findById(id: Long): RaiderEntity? = raiderRepository.findById(id).orElse(null)
}
