...
class ScoreCalculator(
    ...
    fun calculateWithRealData(guildId: String = "default"): List<FlpsBreakdown> {
        ...
        val activity = activityData.characters.find { 
            "${it.characterName}-${it.characterRealm}" == characterKey 
        }
        val wishlist = wishlistData.characters.find { 
            "${it.characterName}-${it.characterRealm}" == characterKey 
        }
        val lootHistory = lootHistoryData.characters.find { 
            "${it.characterName}-${it.characterRealm}" == characterKey 
        }
        val gear = gearData.characters.find { 
            "${it.characterName}-${it.characterRealm}" == characterKey 
        }
        ...
        val ipi = (uv * guildConfig.ipiWeights.upgradeValue) + 
        ...
        val eligible = acs >= guildConfig.thresholds.eligibilityAttendance && 
    ...
    private fun calculateUpgradeValueFromWishlist(wishlist: CharacterWishlistInfo?, gear: CharacterCurrentGear?): Double {
        if (wishlist == null || gear == null) return 0.5 // Default neutral score
        
        // Calculate average upgrade percentage from wishlist items
        val upgradePercentages = wishlist.items.mapNotNull { item ->
    ...
    private fun calculateTierBonusFromGear(gear: CharacterCurrentGear?): Double {
        ...
        val tierPieces = gear.currentGear.count { gearItem ->
    ...
    private fun calculateRoleMultiplier(role: Role, roleMultipliers: RoleMultipliers): Double {
        return when (role) {
