...
class WarcraftLogsSyncController(
    ...
    fun triggerSync(@PathVariable guildId: String): SyncResult = runBlocking {
        syncService.syncReportsForGuild(guildId)
    }
    
    @GetMapping("/sync/{guildId}/status")
    fun getSyncStatus(@PathVariable guildId: String): Map<String, Any> {
    ...
    fun getReports(@PathVariable guildId: String): List<Map<String, Any>> {
