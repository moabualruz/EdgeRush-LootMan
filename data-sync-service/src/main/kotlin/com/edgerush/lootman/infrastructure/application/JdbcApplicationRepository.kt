...
class JdbcApplicationRepository(private val jdbcTemplate: JdbcTemplate) : ApplicationRepository {
    ...
    override fun findByStatus(status: String, offset: Long, limit: Int): List<ApplicationEntity> =
        jdbcTemplate.query("SELECT * FROM applications WHERE status = ? ORDER BY applied_at DESC LIMIT ? OFFSET ?", rowMapper, status, limit, offset)
    ...
    override fun save(entity: ApplicationEntity): ApplicationEntity {
        ...
        if (exists) { update(entity) } else { insert(entity) }
        return entity
    }

    override fun delete(id: Long) { jdbcTemplate.update("DELETE FROM applications WHERE application_id = ?", id) }
