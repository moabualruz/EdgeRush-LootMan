...
class JdbcRaiderTrackItemRepository(private val jdbcTemplate: JdbcTemplate) : RaiderTrackItemRepository {
    ...
    override fun findAll(offset: Long, limit: Int): List<RaiderTrackItemEntity> =
    ...
    override fun findByRaiderId(raiderId: Long, offset: Long, limit: Int): List<RaiderTrackItemEntity> =
        jdbcTemplate.query("SELECT * FROM raider_track_items WHERE raider_id = ? ORDER BY tier LIMIT ? OFFSET ?", rowMapper, raiderId, limit, offset)
    ...
    override fun save(entity: RaiderTrackItemEntity): RaiderTrackItemEntity = if (entity.id == null) insert(entity) else { update(entity); entity }

    override fun delete(id: Long) { jdbcTemplate.update("DELETE FROM raider_track_items WHERE id = ?", id) }
    ...
    private fun update(entity: RaiderTrackItemEntity) {
        jdbcTemplate.update(
            ...
            entity.raiderId, entity.tier, entity.itemCount, entity.id
        )
    }
