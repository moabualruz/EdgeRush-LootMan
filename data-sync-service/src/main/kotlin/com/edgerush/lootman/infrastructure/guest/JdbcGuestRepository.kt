...
class JdbcGuestRepository(private val jdbcTemplate: JdbcTemplate) : GuestRepository {
    ...
    override fun save(entity: GuestEntity): GuestEntity {
        if (existsById(entity.guestId)) {
            jdbcTemplate.update("UPDATE guests SET name=?, realm=?, class=?, role=?, blizzard_id=?, tracking_since=?, synced_at=? WHERE guest_id=?",
            ...
            jdbcTemplate.update("INSERT INTO guests (guest_id, name, realm, class, role, blizzard_id, tracking_since, synced_at) VALUES (?,?,?,?,?,?,?,?)",
    ...
    override fun delete(guestId: Long) { jdbcTemplate.update("DELETE FROM guests WHERE guest_id = ?", guestId) }

    private val rowMapper = RowMapper { rs, _ ->
        val blizzardIdVal = rs.getLong("blizzard_id"); val blizzardId = if (rs.wasNull()) null else blizzardIdVal
        GuestEntity(rs.getLong("guest_id"), rs.getString("name"), rs.getString("realm"), rs.getString("class"),
            rs.getString("role"), blizzardId, rs.getTimestamp("tracking_since")?.toInstant()?.atOffset(ZoneOffset.UTC),
