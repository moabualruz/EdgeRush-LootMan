...
class JdbcLootAwardWishDataRepository(private val jdbcTemplate: JdbcTemplate) : LootAwardWishDataRepository {
    ...
    override fun findByLootAwardId(
        ...
        jdbcTemplate.query(
            "SELECT * FROM loot_award_wish_data WHERE loot_award_id = ? ORDER BY id LIMIT ? OFFSET ?",
            rowMapper,
            lootAwardId,
            limit,
            offset,
    ...
    override fun delete(id: Long) {
        jdbcTemplate.update("DELETE FROM loot_award_wish_data WHERE id = ?", id)
    }
    ...
    private val rowMapper =
        RowMapper { rs, _ ->
            ...
            LootAwardWishDataEntity(
                rs.getLong("id"),
                rs.getLong("loot_award_id"),
                rs.getString("spec_name"),
                rs.getString("spec_icon"),
                getIntOrNull("value"),
            )
        }
}
