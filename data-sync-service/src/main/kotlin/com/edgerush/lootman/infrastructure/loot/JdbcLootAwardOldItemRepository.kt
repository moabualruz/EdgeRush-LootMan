...
class JdbcLootAwardOldItemRepository(private val jdbcTemplate: JdbcTemplate) : LootAwardOldItemRepository {
    ...
    override fun findByLootAwardId(lootAwardId: Long, offset: Long, limit: Int): List<LootAwardOldItemEntity> =
        jdbcTemplate.query("SELECT * FROM loot_award_old_items WHERE loot_award_id = ? ORDER BY id LIMIT ? OFFSET ?", rowMapper, lootAwardId, limit, offset)
    ...
    override fun save(entity: LootAwardOldItemEntity): LootAwardOldItemEntity = if (entity.id == null) insert(entity) else { update(entity); entity }

    override fun delete(id: Long) { jdbcTemplate.update("DELETE FROM loot_award_old_items WHERE id = ?", id) }
    ...
    private fun update(entity: LootAwardOldItemEntity) {
        jdbcTemplate.update(
            ...
            entity.lootAwardId, entity.itemId, entity.bonusId, entity.id
        )
    }
