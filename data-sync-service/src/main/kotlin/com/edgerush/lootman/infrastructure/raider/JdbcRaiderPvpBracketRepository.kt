...
class JdbcRaiderPvpBracketRepository(private val jdbcTemplate: JdbcTemplate) : RaiderPvpBracketRepository {
    ...
    override fun findAll(offset: Long, limit: Int): List<RaiderPvpBracketEntity> =
    ...
    override fun findByRaiderId(raiderId: Long, offset: Long, limit: Int): List<RaiderPvpBracketEntity> =
        jdbcTemplate.query("SELECT * FROM raider_pvp_bracket_stats WHERE raider_id = ? ORDER BY bracket LIMIT ? OFFSET ?", rowMapper, raiderId, limit, offset)
    ...
    override fun save(entity: RaiderPvpBracketEntity): RaiderPvpBracketEntity = if (entity.id == null) insert(entity) else { update(entity); entity }

    override fun delete(id: Long) { jdbcTemplate.update("DELETE FROM raider_pvp_bracket_stats WHERE id = ?", id) }
    ...
    private fun update(entity: RaiderPvpBracketEntity) {
        jdbcTemplate.update(
            ...
            entity.raiderId, entity.bracket, entity.rating, entity.seasonPlayed, entity.weekPlayed, entity.maxRating, entity.id
        )
    }
    ...
    private val rowMapper = RowMapper { rs, _ ->
        ...
        RaiderPvpBracketEntity(
            rs.getLong("id"), rs.getLong("raider_id"), rs.getString("bracket"),
            getIntOrNull("rating"), getIntOrNull("season_played"), getIntOrNull("week_played"), getIntOrNull("max_rating")
        )
    }
}
