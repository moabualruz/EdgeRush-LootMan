...
class JdbcHistoricalActivityRepository(private val jdbcTemplate: JdbcTemplate) : HistoricalActivityRepository {
    ...
    override fun findByCharacterId(characterId: Long, offset: Long, limit: Int): List<HistoricalActivityEntity> =
        jdbcTemplate.query("SELECT * FROM historical_activity WHERE character_id = ? ORDER BY synced_at DESC LIMIT ? OFFSET ?", rowMapper, characterId, limit, offset)
    ...
    override fun findByTeamId(teamId: Long, offset: Long, limit: Int): List<HistoricalActivityEntity> =
        jdbcTemplate.query("SELECT * FROM historical_activity WHERE team_id = ? ORDER BY synced_at DESC LIMIT ? OFFSET ?", rowMapper, teamId, limit, offset)
    ...
    override fun save(entity: HistoricalActivityEntity): HistoricalActivityEntity = if (entity.id == null) insert(entity) else { update(entity); entity }

    override fun delete(id: Long) { jdbcTemplate.update("DELETE FROM historical_activity WHERE id = ?", id) }
    ...
    private fun update(entity: HistoricalActivityEntity) {
        jdbcTemplate.update(
            ...
            entity.characterId, entity.characterName, entity.characterRealm, entity.periodId, entity.teamId, entity.seasonId, entity.dataJson, Timestamp.from(entity.syncedAt.toInstant()), entity.id
        )
    }
