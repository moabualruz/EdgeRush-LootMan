...
class JdbcPeriodSnapshotRepository(private val jdbcTemplate: JdbcTemplate) : PeriodSnapshotRepository {
    ...
    override fun findByTeamId(
        ...
        jdbcTemplate.query(
            "SELECT * FROM period_snapshots WHERE team_id = ? ORDER BY fetched_at DESC LIMIT ? OFFSET ?",
            rowMapper,
            teamId,
            limit,
            offset,
    ...
    override fun delete(id: Long) {
        jdbcTemplate.update("DELETE FROM period_snapshots WHERE id = ?", id)
    }
        ...
        jdbcTemplate.update({ conn ->
            val ps =
                conn.prepareStatement(
                    "INSERT INTO period_snapshots (team_id, season_id, period_id, current_period, fetched_at) VALUES (?,?,?,?,?)",
    ...
    private fun update(entity: PeriodSnapshotEntity) {
        jdbcTemplate.update(
            "UPDATE period_snapshots SET team_id=?, season_id=?, period_id=?, current_period=?, fetched_at=? WHERE id=?",
            entity.teamId,
            entity.seasonId,
            entity.periodId,
            entity.currentPeriod,
            Timestamp.from(entity.fetchedAt.toInstant()),
            entity.id,
        )
    }
    ...
    private val rowMapper =
        RowMapper { rs, _ ->
            ...
            PeriodSnapshotEntity(
                rs.getLong("id"),
                getLongOrNull("team_id"),
                getLongOrNull("season_id"),
                getLongOrNull("period_id"),
                getLongOrNull("current_period"),
