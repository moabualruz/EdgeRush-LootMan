...
class LootAwardOldItemController(private val service: LootAwardOldItemCrudService, private val paginationProperties: PaginationProperties) {
    ...
    ): LootAwardOldItemResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create a loot award old item")
    fun create(
        @Valid @RequestBody request: CreateLootAwardOldItemRequest,
    ): ResponseEntity<LootAwardOldItemResponse> = ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update a loot award old item")
    fun update(
        @PathVariable id: Long,
        @Valid @RequestBody request: UpdateLootAwardOldItemRequest,
    ): LootAwardOldItemResponse = service.update(id, request)
    ...
    fun exists(
        @PathVariable id: Long,
    ): LootAwardOldItemExistsResponse = LootAwardOldItemExistsResponse(service.existsById(id))
