...
class RaiderRaidProgressCrudServiceImpl(private val repository: RaiderRaidProgressRepository) : RaiderRaidProgressCrudService {
    ...
    override fun findAll(pageRequest: PageRequest): PagedResponse<RaiderRaidProgressResponse> {
        ...
        return PagedResponse(repository.findAll(offset, pageRequest.size).map { RaiderRaidProgressResponse.from(it) },
            pageRequest.page, pageRequest.size, repository.count())
    }

    override fun findById(id: Long): RaiderRaidProgressResponse = repository.findById(id)?.let { RaiderRaidProgressResponse.from(it) }
    ...
    override fun update(id: Long, request: UpdateRaiderRaidProgressRequest): RaiderRaidProgressResponse {
    ...
    override fun findByRaiderId(raiderId: Long, pageRequest: PageRequest): PagedResponse<RaiderRaidProgressResponse> {
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(repository.findByRaiderId(raiderId, offset, pageRequest.size).map { RaiderRaidProgressResponse.from(it) },
