...
class HistoricalActivityCrudServiceImpl(private val repository: HistoricalActivityRepository) : HistoricalActivityCrudService {
    override fun findAll(pageRequest: PageRequest): PagedResponse<HistoricalActivityResponse> {
        ...
        return PagedResponse(
            repository.findAll(offset, pageRequest.size).map { HistoricalActivityResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
            repository.count(),
        )
    ...
    override fun findById(id: Long): HistoricalActivityResponse =
        repository.findById(id)?.let { HistoricalActivityResponse.from(it) }
    ...
    override fun findByCharacterId(
        ...
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(
            repository.findByCharacterId(characterId, offset, pageRequest.size).map { HistoricalActivityResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
            repository.countByCharacterId(characterId),
        )
    }
    ...
    override fun findByTeamId(
        ...
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(
            repository.findByTeamId(teamId, offset, pageRequest.size).map { HistoricalActivityResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
