...
class RaiderStatisticsController(private val service: RaiderStatisticsCrudService, private val paginationProperties: PaginationProperties) {
    ...
    fun findAll(@RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<RaiderStatisticsResponse> =
    ...
    fun findById(@PathVariable id: Long): RaiderStatisticsResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create raider statistics")
    fun create(@Valid @RequestBody request: CreateRaiderStatisticsRequest): ResponseEntity<RaiderStatisticsResponse> =
        ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update raider statistics")
    fun update(@PathVariable id: Long, @Valid @RequestBody request: UpdateRaiderStatisticsRequest): RaiderStatisticsResponse =
    ...
    fun exists(@PathVariable id: Long): RaiderStatisticsExistsResponse = RaiderStatisticsExistsResponse(service.existsById(id))

    @GetMapping("/raider/{raiderId}")
    @Operation(summary = "Find raider statistics by raider ID")
    fun findByRaiderId(@PathVariable raiderId: Long): RaiderStatisticsResponse = service.findByRaiderId(raiderId)
}
