...
class WishlistSnapshotController(private val service: WishlistSnapshotCrudService, private val paginationProperties: PaginationProperties) {
    ...
    fun findAll(@RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<WishlistSnapshotResponse> =
    ...
    fun findById(@PathVariable id: Long): WishlistSnapshotResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create a wishlist snapshot")
    fun create(@Valid @RequestBody request: CreateWishlistSnapshotRequest): ResponseEntity<WishlistSnapshotResponse> =
        ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update a wishlist snapshot")
    fun update(@PathVariable id: Long, @Valid @RequestBody request: UpdateWishlistSnapshotRequest): WishlistSnapshotResponse =
    ...
    fun exists(@PathVariable id: Long): WishlistSnapshotExistsResponse = WishlistSnapshotExistsResponse(service.existsById(id))

    @GetMapping("/raider/{raiderId}")
    @Operation(summary = "Find wishlist snapshots by raider ID")
    fun findByRaiderId(@PathVariable raiderId: Long, @RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<WishlistSnapshotResponse> =
    ...
    fun findByTeamId(@PathVariable teamId: Long, @RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<WishlistSnapshotResponse> =
