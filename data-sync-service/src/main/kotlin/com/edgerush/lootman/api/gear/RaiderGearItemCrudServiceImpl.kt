...
class RaiderGearItemCrudServiceImpl(private val repository: RaiderGearItemRepository) : RaiderGearItemCrudService {
    override fun findAll(pageRequest: PageRequest): PagedResponse<RaiderGearItemResponse> {
        ...
        return PagedResponse(
            repository.findAll(offset, pageRequest.size).map { RaiderGearItemResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
            repository.count(),
        )
    ...
    override fun findById(id: Long): RaiderGearItemResponse =
        repository.findById(id)?.let { RaiderGearItemResponse.from(it) }
    ...
    ): PagedResponse<RaiderGearItemResponse> {
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(
            repository.findByRaiderId(raiderId, offset, pageRequest.size).map { RaiderGearItemResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
    ...
    override fun findByRaiderIdAndGearSet(
        ...
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(
            repository.findByRaiderIdAndGearSet(raiderId, gearSet, offset, pageRequest.size).map { RaiderGearItemResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
            repository.countByRaiderIdAndGearSet(raiderId, gearSet),
        )
    }
    ...
    override fun countByRaiderIdAndGearSet(
        raiderId: Long,
        gearSet: String,
    ): Long = repository.countByRaiderIdAndGearSet(raiderId, gearSet)
}
