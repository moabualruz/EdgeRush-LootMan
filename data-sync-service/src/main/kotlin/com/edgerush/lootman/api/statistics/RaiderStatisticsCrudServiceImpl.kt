...
class RaiderStatisticsCrudServiceImpl(private val repository: RaiderStatisticsRepository) : RaiderStatisticsCrudService {
    ...
    override fun findAll(pageRequest: PageRequest): PagedResponse<RaiderStatisticsResponse> {
        ...
        return PagedResponse(repository.findAll(offset, pageRequest.size).map { RaiderStatisticsResponse.from(it) },
            pageRequest.page, pageRequest.size, repository.count())
    }

    override fun findById(id: Long): RaiderStatisticsResponse = repository.findById(id)?.let { RaiderStatisticsResponse.from(it) }
    ...
    override fun create(request: CreateRaiderStatisticsRequest): RaiderStatisticsResponse {
        val entity = RaiderStatisticsEntity(
    ...
    override fun update(id: Long, request: UpdateRaiderStatisticsRequest): RaiderStatisticsResponse {
    ...
    override fun findByRaiderId(raiderId: Long): RaiderStatisticsResponse = repository.findByRaiderId(raiderId)?.let { RaiderStatisticsResponse.from(it) }
