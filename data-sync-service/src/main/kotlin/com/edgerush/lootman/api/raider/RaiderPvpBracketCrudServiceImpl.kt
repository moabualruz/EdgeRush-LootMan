...
class RaiderPvpBracketCrudServiceImpl(private val repository: RaiderPvpBracketRepository) : RaiderPvpBracketCrudService {
    override fun findAll(pageRequest: PageRequest): PagedResponse<RaiderPvpBracketResponse> {
        ...
        return PagedResponse(
            repository.findAll(offset, pageRequest.size).map { RaiderPvpBracketResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
            repository.count(),
        )
    ...
    override fun findById(id: Long): RaiderPvpBracketResponse =
        repository.findById(id)?.let { RaiderPvpBracketResponse.from(it) }
    ...
    override fun findByRaiderId(
        ...
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(
            repository.findByRaiderId(raiderId, offset, pageRequest.size).map { RaiderPvpBracketResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
