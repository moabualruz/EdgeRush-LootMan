...
class ApplicationQuestionFileController(
    private val service: ApplicationQuestionFileCrudService,
    ...
    ): PagedResponse<ApplicationQuestionFileResponse> =
    ...
    ): ApplicationQuestionFileResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create an application question file")
    fun create(
        @Valid @RequestBody request: CreateApplicationQuestionFileRequest,
    ): ResponseEntity<ApplicationQuestionFileResponse> = ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update an application question file")
    fun update(
        @PathVariable id: Long,
        @Valid @RequestBody request: UpdateApplicationQuestionFileRequest,
    ): ApplicationQuestionFileResponse = service.update(id, request)
    ...
    fun exists(
        @PathVariable id: Long,
    ): ApplicationQuestionFileExistsResponse = ApplicationQuestionFileExistsResponse(service.existsById(id))
