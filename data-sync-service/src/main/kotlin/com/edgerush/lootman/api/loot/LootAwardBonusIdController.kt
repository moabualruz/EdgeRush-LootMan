...
class LootAwardBonusIdController(private val service: LootAwardBonusIdCrudService, private val paginationProperties: PaginationProperties) {
    ...
    ): LootAwardBonusIdResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create a loot award bonus ID")
    fun create(
        @Valid @RequestBody request: CreateLootAwardBonusIdRequest,
    ): ResponseEntity<LootAwardBonusIdResponse> = ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update a loot award bonus ID")
    fun update(
        @PathVariable id: Long,
        @Valid @RequestBody request: UpdateLootAwardBonusIdRequest,
    ): LootAwardBonusIdResponse = service.update(id, request)
    ...
    fun exists(
        @PathVariable id: Long,
    ): LootAwardBonusIdExistsResponse = LootAwardBonusIdExistsResponse(service.existsById(id))
