...
class LootAwardWishDataCrudServiceImpl(private val repository: LootAwardWishDataRepository) : LootAwardWishDataCrudService {
    ...
    override fun findAll(pageRequest: PageRequest): PagedResponse<LootAwardWishDataResponse> {
        ...
        return PagedResponse(repository.findAll(offset, pageRequest.size).map { LootAwardWishDataResponse.from(it) },
            pageRequest.page, pageRequest.size, repository.count())
    }

    override fun findById(id: Long): LootAwardWishDataResponse = repository.findById(id)?.let { LootAwardWishDataResponse.from(it) }
    ...
    override fun update(id: Long, request: UpdateLootAwardWishDataRequest): LootAwardWishDataResponse {
    ...
    override fun findByLootAwardId(lootAwardId: Long, pageRequest: PageRequest): PagedResponse<LootAwardWishDataResponse> {
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(repository.findByLootAwardId(lootAwardId, offset, pageRequest.size).map { LootAwardWishDataResponse.from(it) },
