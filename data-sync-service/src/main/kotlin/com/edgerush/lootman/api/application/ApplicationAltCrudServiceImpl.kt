...
class ApplicationAltCrudServiceImpl(private val repository: ApplicationAltRepository) : ApplicationAltCrudService {
    override fun findAll(pageRequest: PageRequest): PagedResponse<ApplicationAltResponse> {
        ...
        return PagedResponse(
            repository.findAll(offset, pageRequest.size).map { ApplicationAltResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
            repository.count(),
        )
    ...
    override fun findById(id: Long): ApplicationAltResponse =
        repository.findById(id)?.let { ApplicationAltResponse.from(it) }
    ...
    override fun findByApplicationId(
        ...
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(
            repository.findByApplicationId(applicationId, offset, pageRequest.size).map { ApplicationAltResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
