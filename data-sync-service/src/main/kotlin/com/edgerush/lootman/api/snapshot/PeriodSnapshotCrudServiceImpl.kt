...
class PeriodSnapshotCrudServiceImpl(private val repository: PeriodSnapshotRepository) : PeriodSnapshotCrudService {
    ...
    override fun findAll(pageRequest: PageRequest): PagedResponse<PeriodSnapshotResponse> {
        ...
        return PagedResponse(repository.findAll(offset, pageRequest.size).map { PeriodSnapshotResponse.from(it) },
            pageRequest.page, pageRequest.size, repository.count())
    }

    override fun findById(id: Long): PeriodSnapshotResponse = repository.findById(id)?.let { PeriodSnapshotResponse.from(it) }
    ...
    override fun create(request: CreatePeriodSnapshotRequest): PeriodSnapshotResponse {
        val entity = PeriodSnapshotEntity(null, request.teamId, request.seasonId, request.periodId, request.currentPeriod, OffsetDateTime.now())
        return PeriodSnapshotResponse.from(repository.save(entity))
    }

    override fun update(id: Long, request: UpdatePeriodSnapshotRequest): PeriodSnapshotResponse {
    ...
    override fun findByTeamId(teamId: Long, pageRequest: PageRequest): PagedResponse<PeriodSnapshotResponse> {
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(repository.findByTeamId(teamId, offset, pageRequest.size).map { PeriodSnapshotResponse.from(it) },
