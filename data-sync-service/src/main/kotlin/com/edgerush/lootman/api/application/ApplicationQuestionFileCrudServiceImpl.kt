...
@Service
class ApplicationQuestionFileCrudServiceImpl(
    private val repository: ApplicationQuestionFileRepository,
) : ApplicationQuestionFileCrudService {
    override fun findAll(pageRequest: PageRequest): PagedResponse<ApplicationQuestionFileResponse> {
        ...
        return PagedResponse(
            repository.findAll(offset, pageRequest.size).map { ApplicationQuestionFileResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
            repository.count(),
        )
    ...
    override fun findById(id: Long): ApplicationQuestionFileResponse =
        repository.findById(id)?.let { ApplicationQuestionFileResponse.from(it) }
    ...
    override fun findByApplicationId(
        ...
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(
            repository.findByApplicationId(applicationId, offset, pageRequest.size).map { ApplicationQuestionFileResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
