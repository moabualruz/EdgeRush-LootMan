...
class RaiderTrackItemController(private val service: RaiderTrackItemCrudService, private val paginationProperties: PaginationProperties) {
    ...
    ): RaiderTrackItemResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create a raider track item")
    fun create(
        @Valid @RequestBody request: CreateRaiderTrackItemRequest,
    ): ResponseEntity<RaiderTrackItemResponse> = ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update a raider track item")
    fun update(
        @PathVariable id: Long,
        @Valid @RequestBody request: UpdateRaiderTrackItemRequest,
    ): RaiderTrackItemResponse = service.update(id, request)
    ...
    fun exists(
        @PathVariable id: Long,
    ): RaiderTrackItemExistsResponse = RaiderTrackItemExistsResponse(service.existsById(id))
