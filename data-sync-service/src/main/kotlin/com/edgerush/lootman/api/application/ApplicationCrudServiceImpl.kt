...
class ApplicationCrudServiceImpl(private val repository: ApplicationRepository) : ApplicationCrudService {
    override fun findAll(pageRequest: PageRequest): PagedResponse<ApplicationResponse> {
        ...
        return PagedResponse(
            repository.findAll(offset, pageRequest.size).map { ApplicationResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
            repository.count(),
        )
    ...
    override fun findById(id: Long): ApplicationResponse =
        repository.findById(id)?.let { ApplicationResponse.from(it) }
    ...
    override fun findByStatus(
        ...
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(
            repository.findByStatus(status, offset, pageRequest.size).map { ApplicationResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
