...
class HistoricalActivityController(
    private val service: HistoricalActivityCrudService,
    ...
    ): HistoricalActivityResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create a historical activity")
    fun create(
        @Valid @RequestBody request: CreateHistoricalActivityRequest,
    ): ResponseEntity<HistoricalActivityResponse> = ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update a historical activity")
    fun update(
        @PathVariable id: Long,
        @Valid @RequestBody request: UpdateHistoricalActivityRequest,
    ): HistoricalActivityResponse = service.update(id, request)
    ...
    fun exists(
        @PathVariable id: Long,
    ): HistoricalActivityExistsResponse = HistoricalActivityExistsResponse(service.existsById(id))
