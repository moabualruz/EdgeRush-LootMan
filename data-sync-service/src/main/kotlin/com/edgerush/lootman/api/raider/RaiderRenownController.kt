...
class RaiderRenownController(private val service: RaiderRenownCrudService, private val paginationProperties: PaginationProperties) {
    ...
    fun findAll(@RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<RaiderRenownResponse> =
    ...
    fun findById(@PathVariable id: Long): RaiderRenownResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create raider renown")
    fun create(@Valid @RequestBody request: CreateRaiderRenownRequest): ResponseEntity<RaiderRenownResponse> =
        ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update raider renown")
    fun update(@PathVariable id: Long, @Valid @RequestBody request: UpdateRaiderRenownRequest): RaiderRenownResponse =
    ...
    fun exists(@PathVariable id: Long): RaiderRenownExistsResponse = RaiderRenownExistsResponse(service.existsById(id))

    @GetMapping("/raider/{raiderId}")
    @Operation(summary = "Find raider renown by raider ID")
    fun findByRaiderId(@PathVariable raiderId: Long, @RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<RaiderRenownResponse> =
