...
class ApplicationController(private val service: ApplicationCrudService, private val paginationProperties: PaginationProperties) {
    ...
    ): ApplicationResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create an application")
    fun create(
        @Valid @RequestBody request: CreateApplicationRequest,
    ): ResponseEntity<ApplicationResponse> = ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))
    ...
    ): ApplicationResponse = service.update(id, request)
    ...
    fun exists(
        @PathVariable id: Long,
    ): ApplicationExistsResponse = ApplicationExistsResponse(service.existsById(id))
