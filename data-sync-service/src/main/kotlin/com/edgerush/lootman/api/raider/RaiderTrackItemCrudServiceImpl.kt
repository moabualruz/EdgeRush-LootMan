...
class RaiderTrackItemCrudServiceImpl(private val repository: RaiderTrackItemRepository) : RaiderTrackItemCrudService {
    ...
    override fun findAll(pageRequest: PageRequest): PagedResponse<RaiderTrackItemResponse> {
        ...
        return PagedResponse(repository.findAll(offset, pageRequest.size).map { RaiderTrackItemResponse.from(it) },
            pageRequest.page, pageRequest.size, repository.count())
    }

    override fun findById(id: Long): RaiderTrackItemResponse = repository.findById(id)?.let { RaiderTrackItemResponse.from(it) }
    ...
    override fun update(id: Long, request: UpdateRaiderTrackItemRequest): RaiderTrackItemResponse {
    ...
    override fun findByRaiderId(raiderId: Long, pageRequest: PageRequest): PagedResponse<RaiderTrackItemResponse> {
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(repository.findByRaiderId(raiderId, offset, pageRequest.size).map { RaiderTrackItemResponse.from(it) },
