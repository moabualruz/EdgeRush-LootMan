...
class RaiderCrestCountController(private val service: RaiderCrestCountCrudService, private val paginationProperties: PaginationProperties) {
    ...
    fun findAll(@RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<RaiderCrestCountResponse> =
    ...
    fun findById(@PathVariable id: Long): RaiderCrestCountResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create a raider crest count")
    fun create(@Valid @RequestBody request: CreateRaiderCrestCountRequest): ResponseEntity<RaiderCrestCountResponse> =
        ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update a raider crest count")
    fun update(@PathVariable id: Long, @Valid @RequestBody request: UpdateRaiderCrestCountRequest): RaiderCrestCountResponse =
    ...
    fun exists(@PathVariable id: Long): RaiderCrestCountExistsResponse = RaiderCrestCountExistsResponse(service.existsById(id))

    @GetMapping("/raider/{raiderId}")
    @Operation(summary = "Find raider crest counts by raider ID")
    fun findByRaiderId(@PathVariable raiderId: Long, @RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<RaiderCrestCountResponse> =
