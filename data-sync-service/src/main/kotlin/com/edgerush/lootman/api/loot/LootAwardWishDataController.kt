...
class LootAwardWishDataController(private val service: LootAwardWishDataCrudService, private val paginationProperties: PaginationProperties) {

    @GetMapping
    @Operation(summary = "Find all loot award wish data")
    fun findAll(@RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<LootAwardWishDataResponse> =
    ...
    fun findById(@PathVariable id: Long): LootAwardWishDataResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create loot award wish data")
    fun create(@Valid @RequestBody request: CreateLootAwardWishDataRequest): ResponseEntity<LootAwardWishDataResponse> =
        ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update loot award wish data")
    fun update(@PathVariable id: Long, @Valid @RequestBody request: UpdateLootAwardWishDataRequest): LootAwardWishDataResponse =
    ...
    fun exists(@PathVariable id: Long): LootAwardWishDataExistsResponse = LootAwardWishDataExistsResponse(service.existsById(id))

    @GetMapping("/loot-award/{lootAwardId}")
    @Operation(summary = "Find loot award wish data by loot award ID")
    fun findByLootAwardId(@PathVariable lootAwardId: Long, @RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<LootAwardWishDataResponse> =
