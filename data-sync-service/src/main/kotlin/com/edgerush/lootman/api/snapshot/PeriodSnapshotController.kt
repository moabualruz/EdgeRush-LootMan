...
class PeriodSnapshotController(private val service: PeriodSnapshotCrudService, private val paginationProperties: PaginationProperties) {
    ...
    ): PeriodSnapshotResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create a period snapshot")
    fun create(
        @Valid @RequestBody request: CreatePeriodSnapshotRequest,
    ): ResponseEntity<PeriodSnapshotResponse> = ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update a period snapshot")
    fun update(
        @PathVariable id: Long,
        @Valid @RequestBody request: UpdatePeriodSnapshotRequest,
    ): PeriodSnapshotResponse = service.update(id, request)
    ...
    fun exists(
        @PathVariable id: Long,
    ): PeriodSnapshotExistsResponse = PeriodSnapshotExistsResponse(service.existsById(id))
