...
class WishlistSnapshotCrudServiceImpl(private val repository: WishlistSnapshotRepository) : WishlistSnapshotCrudService {
    ...
    override fun findAll(pageRequest: PageRequest): PagedResponse<WishlistSnapshotResponse> {
        ...
        return PagedResponse(repository.findAll(offset, pageRequest.size).map { WishlistSnapshotResponse.from(it) },
            pageRequest.page, pageRequest.size, repository.count())
    }

    override fun findById(id: Long): WishlistSnapshotResponse = repository.findById(id)?.let { WishlistSnapshotResponse.from(it) }
    ...
    override fun update(id: Long, request: UpdateWishlistSnapshotRequest): WishlistSnapshotResponse {
    ...
    override fun findByRaiderId(raiderId: Long, pageRequest: PageRequest): PagedResponse<WishlistSnapshotResponse> {
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(repository.findByRaiderId(raiderId, offset, pageRequest.size).map { WishlistSnapshotResponse.from(it) },
            pageRequest.page, pageRequest.size, repository.countByRaiderId(raiderId))
    }

    override fun findByTeamId(teamId: Long, pageRequest: PageRequest): PagedResponse<WishlistSnapshotResponse> {
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(repository.findByTeamId(teamId, offset, pageRequest.size).map { WishlistSnapshotResponse.from(it) },
