...
class RaiderWarcraftLogController(private val service: RaiderWarcraftLogCrudService, private val paginationProperties: PaginationProperties) {

    @GetMapping
    @Operation(summary = "Find all raider Warcraft Logs scores")
    fun findAll(@RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<RaiderWarcraftLogResponse> =
    ...
    fun findById(@PathVariable id: Long): RaiderWarcraftLogResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create a raider Warcraft Logs score")
    fun create(@Valid @RequestBody request: CreateRaiderWarcraftLogRequest): ResponseEntity<RaiderWarcraftLogResponse> =
    ...
    fun update(@PathVariable id: Long, @Valid @RequestBody request: UpdateRaiderWarcraftLogRequest): RaiderWarcraftLogResponse =
    ...
    fun exists(@PathVariable id: Long): RaiderWarcraftLogExistsResponse = RaiderWarcraftLogExistsResponse(service.existsById(id))

    @GetMapping("/raider/{raiderId}")
    @Operation(summary = "Find raider Warcraft Logs scores by raider ID")
    fun findByRaiderId(@PathVariable raiderId: Long, @RequestParam(defaultValue = "0") page: Int, @RequestParam(required = false) size: Int?): PagedResponse<RaiderWarcraftLogResponse> =
