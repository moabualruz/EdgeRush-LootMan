...
class ApplicationAltController(private val service: ApplicationAltCrudService, private val paginationProperties: PaginationProperties) {
    ...
    ): PagedResponse<ApplicationAltResponse> =
    ...
    ): ApplicationAltResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create an application alt")
    fun create(
        @Valid @RequestBody request: CreateApplicationAltRequest,
    ): ResponseEntity<ApplicationAltResponse> = ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update an application alt")
    fun update(
        @PathVariable id: Long,
        @Valid @RequestBody request: UpdateApplicationAltRequest,
    ): ApplicationAltResponse = service.update(id, request)
    ...
    fun exists(
        @PathVariable id: Long,
    ): ApplicationAltExistsResponse = ApplicationAltExistsResponse(service.existsById(id))
