...
class GuestCrudServiceImpl(private val repository: GuestRepository) : GuestCrudService {
    override fun findAll(pageRequest: PageRequest): PagedResponse<GuestResponse> {
        ...
        return PagedResponse(
            repository.findAll(offset, pageRequest.size).map { GuestResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
            repository.count(),
        )
    ...
    override fun findById(id: Long): GuestResponse =
        repository.findById(id)?.let { GuestResponse.from(it) }
