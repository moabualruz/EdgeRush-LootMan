...
class LootAwardBonusIdCrudServiceImpl(private val repository: LootAwardBonusIdRepository) : LootAwardBonusIdCrudService {
    override fun findAll(pageRequest: PageRequest): PagedResponse<LootAwardBonusIdResponse> {
        ...
        return PagedResponse(
            repository.findAll(offset, pageRequest.size).map { LootAwardBonusIdResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
            repository.count(),
        )
    ...
    override fun findById(id: Long): LootAwardBonusIdResponse =
        repository.findById(id)?.let { LootAwardBonusIdResponse.from(it) }
    ...
    override fun findByLootAwardId(
        ...
        val offset = pageRequest.page.toLong() * pageRequest.size
        return PagedResponse(
            repository.findByLootAwardId(lootAwardId, offset, pageRequest.size).map { LootAwardBonusIdResponse.from(it) },
            pageRequest.page,
            pageRequest.size,
