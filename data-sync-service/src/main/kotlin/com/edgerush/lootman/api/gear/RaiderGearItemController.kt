...
class RaiderGearItemController(private val service: RaiderGearItemCrudService, private val paginationProperties: PaginationProperties) {
    ...
    ): RaiderGearItemResponse = service.findById(id)

    @PostMapping
    @Operation(summary = "Create a raider gear item")
    fun create(
        @Valid @RequestBody request: CreateRaiderGearItemRequest,
    ): ResponseEntity<RaiderGearItemResponse> = ResponseEntity.status(HttpStatus.CREATED).body(service.create(request))

    @PutMapping("/{id}")
    @Operation(summary = "Update a raider gear item")
    fun update(
        @PathVariable id: Long,
        @Valid @RequestBody request: UpdateRaiderGearItemRequest,
    ): RaiderGearItemResponse = service.update(id, request)
    ...
    fun exists(
        @PathVariable id: Long,
    ): RaiderGearItemExistsResponse = RaiderGearItemExistsResponse(service.existsById(id))
    ...
    fun findByRaiderIdAndGearSet(
        @PathVariable raiderId: Long,
        @PathVariable gearSet: String,
        @RequestParam(defaultValue = "0") page: Int,
        @RequestParam(required = false) size: Int?,
    ): PagedResponse<RaiderGearItemResponse> =
        service.findByRaiderIdAndGearSet(
            raiderId,
            gearSet,
            PageRequest.withDefaults(page, size, paginationProperties.defaultPageSize, paginationProperties.maxPageSize),
        )
