...
class CreateGuildUseCase(
    ...
    fun execute(command: CreateGuildCommand): Result<Guild> =
        runCatching {
            ...
            val region =
                Region.fromString(command.region)
                    ?: throw IllegalArgumentException("Invalid region: ${command.region}")

            val benchmarkMode =
                BenchmarkMode.fromString(command.benchmarkMode)
...
class UpdateGuildUseCase(
    ...
    fun execute(command: UpdateGuildCommand): Result<Guild> =
        runCatching {
            val guildId = GuildId(command.id)
...
class DeleteGuildUseCase(
    ...
    fun execute(command: DeleteGuildCommand): Result<Unit> =
        runCatching {
            val guildId = GuildId(command.id)
...
class GetGuildUseCase(
    ...
    fun execute(query: GetGuildQuery): Result<Guild> =
        runCatching {
...
class ListGuildsUseCase(
    ...
    fun execute(): Result<List<Guild>> =
        runCatching {
            guildRepository.findAll()
        }

    fun executeActiveOnly(): Result<List<Guild>> =
        runCatching {
