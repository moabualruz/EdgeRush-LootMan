...
const calendarDays = computed(() => {
  ...
  for (let day = 1; day <= lastDay.getDate(); day++) {
    ...
    const record = attendanceData.value?.records?.find(
      (r) => r.raidDate.split("T")[0] === dateStr,
    );
    ...
    <div v-else-if="attendanceData" class="space-y-6">
      ...
      <div v-else class="card">
        ...
        <div class="flex flex-wrap gap-4 mt-4 pt-4 border-t border-gray-700">
          ...
          >
            <span
              :class="['w-3 h-3 rounded-full', getStatusColor(status)]"
            ></span>
      ...
      <!-- Last Raid Info -->
      <div
        v-if="attendanceData.lastRaidDate"
