...
<template>
  <div class="space-y-8">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-bold tracking-tight text-white mb-2 text-glow">Mission Control</h1>
        <p class="text-muted-foreground">Your performance command center for {{ guildContextStore.activeGuild?.guildName || 'your guild' }}</p>
      </div>
      <div v-if="flpsData" class="flex gap-2">
        <span class="px-3 py-1 rounded-full text-xs font-medium border border-border bg-black/40 text-muted-foreground">
          Updated: Today
        </span>
      </div>
    </div>
    ...
    <!-- No Guild state -->
    <div v-else-if="!guildId" class="glass-card p-8 text-center border-primary/20">
      <div class="inline-flex p-4 rounded-full bg-primary/10 mb-4">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
      </div>
      <h2 class="text-xl font-bold text-white mb-2">Join a Guild</h2>
      <p class="text-muted-foreground max-w-md mx-auto mb-6">
        You are not currently a member of any guild. Join a guild to start tracking your performance and loot eligibility.
      </p>
    ...
    <!-- Error state -->
    <div v-else-if="flpsError" class="glass-card p-6 border-destructive/50 bg-destructive/5">
      <div class="flex items-center gap-3 text-destructive-foreground">
        ...
        <p class="font-medium">Failed to load performance data. The sync service might be offline.</p>
      </div>
    </div>

    <!-- Main Content -->
    <div v-else-if="flpsData" class="space-y-8 animate-fade-in-up">
      ...
      >
        ...
        <div>
          <h3 class="font-semibold text-yellow-400 mb-1">Loot Eligibility Warning</h3>
      ...
      <div class="glass-card p-6">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg font-semibold text-white flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
            Recent Loot History
          </h2>
          <RouterLink to="/history" class="text-xs text-muted-foreground hover:text-white transition-colors">View All</RouterLink>
        </div>
        ...
        <p v-else class="text-muted-foreground text-center py-8">No loot history recording in the last 30 days.</p>
      </div>
    </div>
  </div>
</template>
