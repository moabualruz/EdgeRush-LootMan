...
export const flpsApi = {
  async getMyFlps(guildId: string): Promise<FlpsScore> {
    const response = await api.get<FlpsScore>(`/v1/flps/guilds/${guildId}/me`)
    return response.data
  },

  async getFlpsReport(guildId: string): Promise<FlpsReport> {
    const response = await api.get<FlpsReport>(`/v1/flps/guilds/${guildId}/report`)
    return response.data
  },
  ...
  async getLeaderboard(guildId: string, role?: string, limit = 50): Promise<LeaderboardResponse> {
    ...
    const response = await api.get<LeaderboardResponse>(
      `/v1/flps/guilds/${guildId}/leaderboard?${params.toString()}`
    )
    return response.data
  },

  async getConfig(guildId: string): Promise<FlpsConfig> {
    const response = await api.get<FlpsConfig>(`/v1/flps/guilds/${guildId}/config`)
    return response.data
  },

  async previewConfig(guildId: string, config: Partial<FlpsConfig>) {
    const response = await api.post(`/v1/flps/guilds/${guildId}/config/preview`, config)
    return response.data
  },

  async updateConfig(guildId: string, config: Partial<FlpsConfig>): Promise<FlpsConfig> {
    const response = await api.put<FlpsConfig>(`/v1/flps/guilds/${guildId}/config`, config)
    return response.data
  },
}
